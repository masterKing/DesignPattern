...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    self.array = self.window.rootViewController.childViewControllers;
    return YES;
}
...
- (void)buttonClicked:(UIButton *)sender{
    ...
    if (self.window.rootViewController.childViewControllers.count == 3) {
        NSMutableArray *array = self.window.rootViewController.childViewControllers.mutableCopy;
        [array removeLastObject];
        UITabBarController *t = (UITabBarController *)self.window.rootViewController;
        [t setViewControllers:array animated:NO];
    }else{
        UITabBarController *t = (UITabBarController *)self.window.rootViewController;
        [t setViewControllers:self.array animated:NO];
    }
}
